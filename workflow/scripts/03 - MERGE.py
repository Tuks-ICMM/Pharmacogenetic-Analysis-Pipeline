...
# Fishers(supplementary["SUPER"], refPop, populations)
...
#         join("..", "final", "Supplementary Table", "SUPER", "{}_FishersP.csv".format(gene)),
#         sep="\t",
#         index=False,
#     )
#     supplementary["SUPER"][gene]["OR"].to_csv(
#         join("..", "final", "Supplementary Table", "SUPER", "{}_FishersOR.csv".format(gene)),
#         sep="\t",
#         index=False,
#     )
...
for file in glob.glob("../final/Supplementary Table/*/*_*.csv"):
    ...
    data[cluster][gene][test] = pd.read_csv(
        "../final/Supplementary Table/{cluster}/{gene}_{test}.csv".format(
            cluster=g.group(1), gene=gene, test=test
        ),
        delimiter="\t",
    )
...
for cluster in clusters:
    for gene in genes:
        with pd.ExcelWriter(
            "../final/{cluster}-{gene}.xlsx".format(cluster=cluster, gene=gene),
            engine="xlsxwriter",
        ) as writer:
...
probabilities = {
    "SIFT": pd.read_csv(
        "../binaries/CONDEL/sift.data",
        delimiter="\t",
        names=["Score", "Deleterious", "Normal"],
    ),
    "PolyPhen": pd.read_csv(
        "../binaries/CONDEL/polyphen.data",
        delimiter="\t",
        names=["Score", "Deleterious", "Normal"],
    ),
}
...
data = dict()
for gene in genes:
    data[gene] = read_vcf("../final/ALL_{gene}.vcf.gz".format(gene=gene))
